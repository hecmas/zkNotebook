...
const multivariate_polynomials_1 = require("./multivariate_polynomials");
...
class Prover {
    ...
    constructor(p, pol, verbose = false) {
        const MPRp = new multivariate_polynomials_1.MultivariatePolynomialRing(p); // the ring of polynomials over Fp
        const Fp = MPRp.Fp; // the prime field over which the polynomials are defined
        this.Fp = Fp;
        this.MPRp = MPRp;
        this.nvars = (0, multivariate_polynomials_1.count_number_of_variables)(pol);
        this.pols = [pol];
    ...
    compute_and_send_next_polynomial(V) {
        ...
        let g_j = new multivariate_polynomials_1.ArrayMap([[[], 0n]]);
    ...
    prepare_next_polynomial(c) {
        ...
        const nvar = (0, multivariate_polynomials_1.count_number_of_variables)(current_pol);
...
class Verifier {
    ...
    receive_polynomial(p) {
        ...
        if (this.verbose) {
            console.log(`V received polynomial g_${this.round}(X) = ${this.MPRp.toString(p)} in round ${this.round}`);
        }
    }
    ...
    check_gj_is_correct() {
        ...
        const deg_gj = (0, multivariate_polynomials_1.degree)(gj);
        const deg_j_g = (0, multivariate_polynomials_1.degree_j)(this.pol, this.nvars - (this.round - 1));
        if (deg_gj > deg_j_g) {
...
class SumCheckProtocol {
    ...
    constructor(p, g, verbose = false) {
        const nvars = (0, multivariate_polynomials_1.count_number_of_variables)(g);
        if (nvars < 1) {
...
// Main
const p = 97n;
const g = new multivariate_polynomials_1.ArrayMap();
g.set([3, 0, 0], 2n);
g.set([1, 0, 1], 1n);
g.set([0, 1, 1], 1n);
