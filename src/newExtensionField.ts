...
//     // Constructor
//     constructor(Fp: PrimeField, modulusCoeffs: bigint[], trailing_zeros: boolean = false) {
//         // The prime field over which the extension is defined
//         this.Fp = Fp;
...
//         // The degree of the extension
//         let d = modulusCoeffs.length - 1;
//         while (d && modulusCoeffs[d] === 0n) {
//             // modulusCoeffs.pop();
//             d--;
//         }
//         // this.extension_degree = modulusCoeffs.length - 1;
//         this.extension_degree = d;
...
//     degree(a: bigint[]): number {
//         const l = a.length;
//         let d = l-1;

//         for (let i = l - 1; i >= 0; i --) {
//             if (this.Fp.neq(a[i],this.Fp.zero)) {
//                 return d;
//             }
//             d--;
//         }
...
//     euclidean_division(a: bigint[], b: bigint[]): [bigint[], bigint[]] {
...
//             q[i] = this.Fp.div(r[i + degb], b[degb]);
//             for (let j = 0; j < degb + 1; j++) {
...
//             const diff = new Array<bigint>(this.extension_degree - degr - 1).fill(0n);
...
//     squareAndMultiply(base: bigint[], exponent: bigint): bigint[] {
...
//                 const diff = new Array<bigint>(this.extension_degree - degc - 1).fill(0n);
...
//     neg(a: bigint[]): bigint[] {
//         const c = new Array<bigint>(this.degree(a) + 1);
...
//                     const diff = new Array<bigint>(this.extension_degree - 1).fill(0n);
//                     return [this.Fp.mul(a[0], b[0])].concat(diff);
//                 }
//                 return [this.Fp.mul(a[0], b[0])];
//             } else {
//                 const c = new Array<bigint>(degb + 1);
...
//             if (this.trailing_zeros) {
//                 const diff = new Array<bigint>(this.extension_degree - 1).fill(0n);
//                 return [this.Fp.div(a[0], b[0])].concat(diff);
//             }
//             return [this.Fp.div(a[0], b[0])];
//         } else if (degb === 0) {
//             if (b[0] === 0n) throw new Error("Division by zero");
...
// export class ExtensionFieldOverFq2 implements FiniteField<bigint[]> 
// {
//     readonly Fp: PrimeField;
//     readonly Fq: ExtensionField;
//     readonly modulusCoeffs: bigint[];
//     readonly base_degree: number;
//     readonly extension_degree: number;
...
//     // Public Accessors
//     get zero(): bigint[] {
//         return new Array<bigint>(this.base_degree).fill(0n);
//     }

//     get one(): bigint[] {
//         return [1n,...new Array<bigint>(this.base_degree-1).fill(0n)];
//     }
...
//         let q = new Array<bigint>(this.base_degree*(dega - degb + 1)).fill(0n);
//         let r = a.slice();
//         for (let i = dega - degb; i >= 0; i--) {
...
//     egcd(a: bigint[], b: bigint[]) : [bigint[], bigint[], bigint[]] {
//         let [old_r, r] = [a, b];
//         let [old_s, s] = [this.one, this.zero];
//         let [old_t, t] = [this.zero, this.one];
...
//         const dega = this.degree(a);
//         if (dega < this.extension_degree) {
//             const c = new Array<bigint>(this.base_degree*(dega + 1)).fill(0n);
//             for (let i = 0; i < dega+1; i++) {
...
//         const degb = this.degree(b);
//         let maxdeg = Math.max(dega, degb);
//         const c = new Array<bigint>(this.base_degree*(maxdeg + 1)).fill(0n);
//         for (let i = 0; i < maxdeg + 1; i++) {
...
//         const c = new Array<bigint>(this.base_degree*(d + 1)).fill(0n);
//         for (let i = 0; i < d + 1; i++) {
...
//             } else {
//                 const c = new Array<bigint>(this.base_degree*(degb + 1)).fill(0n);
//                 for (let i = 0; i < degb + 1; i++) {
...
//             return this.mod(c);
//         } else {
//             const c = new Array<bigint>(this.base_degree*(dega + degb + 1)).fill(0n);
//             for (let i = 0; i < dega + 1; i++) {
//                 const acoeff = a.slice(i * this.base_degree, (i + 1) * this.base_degree);
//                 for (let j = 0; j < degb + 1; j++) {
...
//         } else if (degb === 0) {
//             if (this.eq(b,this.zero)) throw new Error("Division by zero");
//             const c = new Array<bigint>(this.base_degree*(dega + 1)).fill(0n);
//             for (let i = 0; i < dega + 1; i++) {
//                 const acoeff = a.slice(i * this.base_degree, (i + 1) * this.base_degree);
...
// const Fp6a = new ExtensionFieldOverFq(Fp2, [[-9n, -1n], [0n,0n], [0n,0n], [1n,0n]]);
...
// const q = [ [ 9n, 3n ], [ 6n, 4n ], [ 6n, 0n ] ]
// // console.log(Fp6a.mul(a,b));
// console.log(Fp6a.inv(b));
// const c = [10n, 2n,5n, 13n,7n, 5n];
// const d = [9n, 1n,1n, 2n,14n, 2n];
// const e = [ 5n, 0n, 8n, 1n, 0n, 5n ];
// // console.log(Fp6b.div(c,d));
// console.log(Fp6b.inv(d));
// TOFIX
